...
- `src/awst/screens/` holds one Textual `Screen` per page (`home.py`, `stacks.py`, `buckets.py`, `functions.py`, `queues.py`, `profiles.py` for the profile picker, `sso_login.py` for the SSO login modal), with list screens subclassing `ResourceListScreen` (`resource_list.py`), and pure presentation helpers (`formatting.py`). Screens load data with thread workers (`@work(thread=True, exclusive=True, exit_on_error=False)`) and handle results in `on_worker_state_changed`.
